...
content = content.replace(
    'def _agent_day(agent: dict, world: dict, day: int, austerity: bool = False, all_agents: list = None) -> str:',
...
if old_llm_block in content:
    ...
    print("COULD NOT FIND LLM BLOCK")
...
# 3. Update _team_meeting
content = content.replace(
    'def _team_meeting(agents: list, world: dict, day: int) -> list:',
    'def _team_meeting(agents: list, world: dict, day: int, day_data: dict = None) -> list:'
)

# Insert the decide_meeting_narrative call
old_threshold_block = '''            # Threshold crossing — log once per pair (using A's view)
...
if old_threshold_block in content:
    ...
    print("COULD NOT FIND THRESHOLD BLOCK")

# 4. Update run_world_for loop
old_day_loop = '''        austerity = world["company_budget"] < AUSTERITY_THRESHOLD
...
if old_day_loop in content:
    content = content.replace(old_day_loop, new_day_loop)
else:
    print("COULD NOT FIND DAY LOOP")
    
# Save day_data at the end of the loop iteration
old_end_loop = '''        if world["current_day"] % 30 == 0:
...
if old_end_loop in content:
    content = content.replace(old_end_loop, new_end_loop)
else:
    print("COULD NOT FIND END LOOP")
